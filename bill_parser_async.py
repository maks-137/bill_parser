...
async def _create_iter_tasks(search_parameters: BillSearchParameters, parse_parameters: BillParseParameters,
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for n in range(1, int(page_number)-160):
            ...
            task = asyncio.create_task(_create_parse_tasks(1, session, search_parameters, parse_parameters,
                                                           download_parameters))

            iter_tasks.append(task)

        bill_items = await asyncio.gather(*iter_tasks)
...
def run(search_parameters: BillSearchParameters, parse_parameters: BillParseParameters,
    ...
    loop.run_until_complete(_create_iter_tasks(search_parameters, parse_parameters, download_parameters))
